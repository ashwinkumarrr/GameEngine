...
Ray CollisionDetection::ScreenPosToWorldRay(glm::vec2 mouseCoords_, glm::vec2 screenSize_, Camera* camera_)
{
	...
	return Ray(glm::vec3(rayStart_World), rayDirection_World);
...
bool CollisionDetection::RayObbIntersection(Ray* ray_, BoundingBox* box_)
{
	...
	//X Axis
	glm::vec3 xAxis((box_->transform[0].x, box_->transform[0].x, box_->transform[0].x));
	...
	//Y Axis 
	glm::vec3 yAxis((box_->transform[0].y, box_->transform[0].y, box_->transform[0].y));
	float d = glm::dot(yAxis, delta);
	float g = glm::dot(ray_->direction, yAxis);
	if (fabs(f) > 0.001f) {
	...
	//Z Axis
	glm::vec3 zAxis((box_->transform[0].z, box_->transform[0].z, box_->transform[0].z));
	float i = glm::dot(zAxis, delta);
	float h = glm::dot(ray_->direction, zAxis);
	if (fabs(f) > 0.001f) {
